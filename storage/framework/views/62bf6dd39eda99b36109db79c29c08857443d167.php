<?php echo $__env->make('user.components.user_head', ['title' => $title ?? 'Identity Verification'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
...
<div class="min-h-screen bg-gray-50 pb-24">
    <div class="max-w-4xl mx-auto px-6 pt-12">
            ...
            <div class="mb-10 bg-white rounded-[32px] p-8 shadow-xl border border-gray-100 relative overflow-hidden">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-6 relative">
                    <div class="flex items-center gap-5">
                        <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-xl <?php echo e($verification->status == 'approved' ? 'bg-emerald-50 text-emerald-600' : ($verification->status == 'rejected' ? 'bg-red-50 text-red-600' : 'bg-orange-50 text-orange-600 animate-pulse')); ?>">
                            <?php if($verification->status == 'approved'): ?> <i class="fa-solid fa-circle-check"></i>
                            <?php elseif($verification->status == 'rejected'): ?> <i class="fa-solid fa-circle-xmark"></i>
                            <?php else: ?> <i class="fa-solid fa-hourglass-half"></i> <?php endif; ?>
                        </div>
                        <div>
                            <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Current Status</div>
                            <div class="text-xl font-black text-gray-900 uppercase tracking-tighter"><?php echo e($verification->status); ?></div>
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Submitted On</div>
                        <div class="text-sm font-bold text-gray-900"><?php echo e(optional($verification->submitted_at)->format('M d, Y') ?? 'Recently'); ?></div>
                    </div>
                </div>
                <?php if($verification->status == 'rejected'): ?>
                    <div class="mt-6 p-4 bg-red-50 rounded-2xl border border-red-100">
                        ...
                        <p class="text-xs font-bold text-red-600"><?php echo e($verification->admin_notes ?? 'Incomplete or unclear documentation. Please try again with clear images.'); ?></p>
                    </div>
                <?php endif; ?>
            </div>
        <?php endif; ?>
        ...
        <div class="bg-white rounded-[40px] shadow-2xl shadow-blue-500/5 border border-gray-100 p-8 md:p-12">
            <form action="<?php echo e(route('user.verify.store')); ?>" method="post" enctype="multipart/form-data" class="space-y-8">
                <?php echo csrf_field(); ?>
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-[9px] text-gray-400 font-bold px-2 italic">Data is encrypted and stored according to <?php echo e($setting->site_name ?? 'our'); ?> global privacy standards.</p>
                </div>
...
<?php /**PATH C:\xampp\htdocs\Coin-trade\resources\views/user/pages/verify.blade.php ENDPATH**/ ?>