<?php echo $__env->make('user.components.user_head', ['title' => 'Send Assets'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    ...
    <div class="send-body">
        ...
        <?php if(session('success')): ?>
            <div class="slide-up" style="background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.2);border-radius:12px;padding:14px 18px;margin-bottom:24px;display:flex;align-items:center;gap:10px;">
                ...
                <span style="font-size:13px;color:#6ee7b7;"><?php echo e(session('success')); ?></span>
            </div>
        <?php endif; ?>

        <form id="sendForm" action="<?php echo e(url('send-crypto')); ?>" method="POST" onsubmit="return handleSend(event)">
            <?php echo csrf_field(); ?>
            ...
            <div class="field-group">
                ...
                <div class="input-surface">
                    <select class="asset-select" id="assetSelect" onchange="onAssetChange(this)">
                        <?php $__empty_1 = true; $__currentLoopData = $userWallets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $wallet): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <option value="<?php echo e($wallet->walletType->id); ?>"
                                    data-balance="<?php echo e($wallet->amount); ?>"
                                    data-symbol="<?php echo e(strtoupper($wallet->walletType->short_code)); ?>"
                                    data-name="<?php echo e($wallet->walletType->coin_name); ?>">
                                <?php echo e($wallet->walletType->coin_name); ?> (<?php echo e(strtoupper($wallet->walletType->short_code)); ?>) — <?php echo e(number_format($wallet->amount, 8)); ?> Available
                            </option>
...
let currentSymbol  = '';
let pinValue       = '';
const correctPin   = '<?php echo e(auth()->user()->pin ?? "0000"); ?>'; // use user's stored PIN if available
...
function checkPin() {
    ...
    const userPin = '<?php echo auth()->user()->transaction_pin ?? ""; ?>';
...
<?php /**PATH C:\xampp\htdocs\Coin-trade\resources\views/user/pages/send-crypto.blade.php ENDPATH**/ ?>