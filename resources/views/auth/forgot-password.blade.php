...
<head>
    ...
    <title>QFSWORLD | Forgot Password</title>
...
<body>
    ...
    <div style="position:relative;z-index:1;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px 16px;">
        ...
        <div class="auth-card" style="width:100%;max-width:420px;padding:36px 32px;">
            ...
            <div style="width:52px;height:52px;background:rgba(37,99,235,0.15);border:1px solid rgba(59,130,246,0.2);border-radius:14px;display:flex;align-items:center;justify-content:center;margin-bottom:18px;">
                <i class="fas fa-key" style="color:#3b82f6;font-size:20px;"></i>
            </div>

            <h2 style="font-size:22px;font-weight:700;color:#f1f5f9;margin-bottom:6px;">Forgot Password?</h2>
            <p style="font-size:13px;color:#64748b;margin-bottom:26px;line-height:1.6;">No worries! Enter your email and we'll send you a password reset link.</p>

            @if (session('status'))
            <div style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.2);border-radius:10px;padding:12px 16px;margin-bottom:20px;display:flex;align-items:center;gap:10px;">
                <i class="fas fa-check-circle" style="color:#22c55e;font-size:14px;"></i>
                <span style="font-size:13px;color:#86efac;">{{ session('status') }}</span>
            </div>
            @endif
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (el && el.innerHTML.trim() !== '') swal("Error", el.innerText.trim(), "error");
        });
    </script>
