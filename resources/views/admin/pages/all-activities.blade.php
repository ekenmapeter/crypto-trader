      ...
      <div class="flex h-screen antialiased text-gray-900 md:bg-white dark:bg-dark dark:text-light  mb-6">
        ...
        <section>
            <div class="md:flex grid-cols-1 md: w-1/2 lg:1/2 w-1/2 items-center md:h-12 md:p-8 m-6 p-4 bg-violet-400 shadow rounded-lg">
                ...
                <div>
                  ...
                  <span class="inline-block text-xl text-white font-bold">{{ $totalActivities }}</span>
                </div>
              </div>
        </section>
        ...
        <section>
            <div class="mt-0 md:m-12 m-2">
               <h3 class="mt-6 text-xl m-6">All Activities</h3>
                      ...
                      <div class="overflow-hidden border-b border-gray-200 rounded-md shadow-md">
                        <table class="min-w-full overflow-x-scroll divide-y divide-gray-200">
                          <thead class="bg-indigo-300">
                           ...
                           @forelse($activitiesTable as $data)
                          <tbody class="bg-white divide-y divide-gray-200">
                              <tr class="transition-all hover:bg-indigo-300 hover:shadow-lg">
                                <th scope="row" class="flex items-center py-4 px-6 text-gray-900 whitespace-nowrap dark:text-white">
                              <div class="pl-3">
                                  <div class="text-xs font-bold"><a href="/preview-activities/{{ $data->id }}">{{ Str::limit($data->subject ,30) }}</a></div>
            
                              </div>  
                          </th>
                          <td class="px-6 py-4 whitespace-nowrap">
                              <div class="text-xs font-semibold"><a href="/preview-activities/{{ $data->id }}">{{ Str::limit($data->message ,70) }}</a></div>
                              </div>
                          </td>
                          <td class="px-6 py-4 whitespace-nowrap">
