@include('user.components.user_head', ['title' => $coin->coin_name . ' Preview'])
        ...
        <div class="bg-white text-black border border-gray-200 shadow-md rounded-xl p-5 md:p-6 mb-6">
            <div class="flex items-start md:items-center justify-between gap-4 flex-col md:flex-row">
                <div class="flex items-center gap-3">
                    <img class="w-12 h-12 rounded-full" src="/images/crypto_logo/{{ $coin->logo }}"
                        alt="{{ $coin->coin_name }}" />
                    <div>
                        <div class="text-sm text-gray-500">{{ $coin->coin_name }}</div>
                        <div class="text-2xl font-semibold tracking-tight">{{ $coin->short_code }}</div>
                    </div>
                </div>
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 w-full md:w-auto">
                    <div class="bg-white/5 border border-gray-200 rounded-lg p-4">
                        ...
                        <div class="text-lg font-semibold">{{ number_format($user_amount, 4) }} {{ $coin->short_code }}
                        </div>
                    </div>
                    <div class="bg-white/5 border border-gray-200 rounded-lg p-4">
                        <div class="text-xs text-gray-500">USD Value</div>
                        <div class="text-lg font-semibold">$ {{ number_format($totalWalletAmount, 2) }}</div>
                    </div>
                            ...
                            <div class="fixed inset-0 overflow-y-auto">
                                <div class="flex min-h-full items-end md:items-center justify-center p-4 text-center">
                                    <div x-show="open" @click.away="open=false"
                                        ...
                                        <div class="p-6">
                                            @if ($charge_fee === 0 || $charge_fee === null)
                                                <div x-data="app()" x-cloak>
                                                    <div x-show.transition="step === 'complete'"
                                                        ...
                                                        <p class="text-center text-gray-500 mb-4">Pay <span
                                                                class="text-blue-400 font-semibold">{{ number_format($coin->restrict, 8) }}
                                                                {{ $coin->coin_name }}</span> to continue your transfer.
                                                        </p>
                                                        <a href="/charge-fee/{{ $coin->id }}" @click="step=1"
                                            ...
                                            @else
                                                <form class="space-y-5" method="POST"
                                                    action="{{ url('send-crypto') }}" enctype="multipart/form-data">
                                                    @csrf
                                                    <div class="flex items-center gap-2">
                                                        <img class="w-8 h-8 rounded-full"
                                                            src="/images/crypto_logo/{{ $coin->logo }}"
                                                            alt="" />
                                                        <label class="text-sm">Edit {{ $coin->coin_name }}</label>
                                                    </div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <input type="number" name="amount"
                                                            class="w-full bg-white/5 border border-gray-200 rounded px-3 py-2 text-black"
                                                            placeholder="Amount in {{ $coin->coin_name }}" required>
                                                    </div>
                                        ...
                                        <div class="p-6">
                                            <div class="flex justify-center items-center gap-2 mb-3">
                                                <img class="w-10 h-10 rounded-full"
                                                    src="/images/crypto_logo/{{ $coin->logo }}" alt="" />
                                                <div class="text-black font-medium">{{ $coin->coin_name }}</div>
                                            </div>
                                            <div class="flex justify-center mb-4">
                                                <img class="rounded w-28 h-28"
                                                    src="/images/crypto_logo/{{ $getwallettype->payment_qr_code }}"
                                                    alt="{{ $coin->coin_name }}" />
                                            </div>
                                            <div class="wallet-container mt-4">
                                                <span id="wallet-address-{{ $getwallettype->id }}"
                                                    class="wallet-address bg-indigo-500 px-2 py-2 rounded text-black font-extrabold">{{ $getwallettype->payment_wallet_address }}</span>
                                                <button
                                                    class="copy-btn float-right rounded bg-indigo-700 text-black font-bold p-2 "
                                                    onclick="copyToClipboard({{ $coin->id }})">Copy</button>
                                            </div>
                                            <p class="text-xs text-gray-500 mt-4 text-center">Send only
                                                {{ $coin->coin_name }} ({{ $coin->short_code }}) to this address.
