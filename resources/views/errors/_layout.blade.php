{{-- Shared error layout partial --}}
{{-- Variables expected: $code, $icon, $title, $message, $accentColor (hex), $accentGradient (CSS gradient) --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ $code }} — {{ $title }} | QFSWORLD</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .blob-2 { width: 500px; height: 500px; background: {{ $accentColor ?? '#7c3aed' }}; bottom: -150px; right: -100px; animation: drift2 16s ease-in-out infinite alternate; }
        ...
        .status-bar { position: fixed; top: 0; left: 0; right: 0; height: 3px; z-index: 20; background: {{ $accentGradient ?? 'linear-gradient(90deg, #2563eb, #3b82f6)' }}; }
        ...
        .error-code {
            ...
            background: {{ $accentGradient ?? 'linear-gradient(135deg,#2563eb,#3b82f6)' }};
            -webkit-background-clip: text; -webkit-text-fill-color: transparent;
            background-clip: text; letter-spacing: -4px; margin-bottom: 8px;
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 0 0 rgba(37,99,235,0.2);}50%{box-shadow:0 0 30px 8px rgba(37,99,235,0.1);} }
        .icon-emoji { font-size: 32px; }

        .error-title { font-size: 28px; font-weight: 800; color: #f1f5f9; margin-bottom: 12px; line-height: 1.2; }
        ...
        .suggestion-row:hover { background: rgba(59,130,246,0.07); border-color: rgba(59,130,246,0.15); }
        .suggestion-icon { width: 36px; height: 36px; background: rgba(37,99,235,0.15); border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: 16px; }
        .suggestion-text { font-size: 13px; color: #94a3b8; }
        .suggestion-text strong { color: #e2e8f0; display: block; margin-bottom: 2px; }

        @media (max-width: 600px) {
            .card { padding: 40px 24px; margin: 16px; }
